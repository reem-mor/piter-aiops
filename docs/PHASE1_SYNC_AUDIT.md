...
|--------|--------|----------|
| **EC2 backend + SPA** | **In sync with local repo** | Served SPA assets match local build: `index-Cfs3WLaY.js`, `index-LqVZ4E_C.css`. `/api/health?deep=1` → all checks ok. `/api/bootstrap` → `kb_id: ${PITER_BEDROCK_KB_ID}`, `rag_backend: agent`, `use_bedrock: true`. |
...
|----------|--------|----------|
| Agent `${PITER_BEDROCK_AGENT_ID}` | PREPARED | `get-agent` — `incidentiq-triage-agent`, prepared 2026-06-09 |
| Alias `${PITER_BEDROCK_AGENT_ALIAS_ID}` | PREPARED | Routes to agent version **21** |
| KB `${PITER_BEDROCK_KB_ID}` | ENABLED on agent DRAFT | `list-agent-knowledge-bases` |
...
|-------------|--------|----------|
| KB attached to agent | **Met** | AWS: KB `${PITER_BEDROCK_KB_ID}` ENABLED on agent `${PITER_BEDROCK_AGENT_ID}` |
...
| EC2 deployment | **Met** | http://localhost:8080/ — deep health ok |
