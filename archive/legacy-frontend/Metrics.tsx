...
export function MetricsPage() {
  return (
    <div className="grid-stack">
      <h1 style={{ margin: 0, fontSize: "1.25rem" }}>Metrics & Tools</h1>
      <p style={{ margin: 0, fontSize: "0.8125rem", color: "var(--text-muted)" }}>
        Frozen GET /api/metrics/* endpoints only.
      </p>
...
function DeploymentsCard() {
  ...
  const run = async () => {
    ...
    try {
      setResult(
        await fetchRecentDeployments({ service, environment, alert_time: alertTime }),
      );
    } catch (e) {
